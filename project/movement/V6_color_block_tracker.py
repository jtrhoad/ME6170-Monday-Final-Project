    ...
    def _run_avoiding(self, target):
        ...
        if self.avoid_phase == 0:
            if elapsed < AVOID_SIDE_DURATION:
                # Mecanum strafe RIGHT (verified on Yahboom Raspbot V2):
                #   FL: forward,  FR: backward,  RL: backward,  RR: forward
                #   = (+S, -S, -S, +S)
                # Multiply by `d` to flip for left strafe (d = -1).
                #
                # NOTE: A previous version of this code used (-S,+S,+S,-S),
                # which physically strafes LEFT on this hardware -- causing
                # the bot to drive INTO obstacles it had correctly identified
                # to its left. The pattern below is verified correct.
                s = AVOID_SIDE_SPEED
                self._drive( s * d, -s * d,
                            -s * d,  s * d)
            else:
