...
ARM_DOWN           = 170            # arm retracted / carrying position (degrees)
...
# ===========================================================================
# ARM CONTROLLER
# Manages the arm (GPIO 18) and claw (GPIO 19) servos for grabbing and
# placing blocks. Uses gpiozero's AngularServo for PWM control.
# ===========================================================================
...
class ArmController:
    ...
    def grab_sequence(self):
        """
        Extend arm, open claw, wait for user to place block, close claw,
...
class ColorBlockTracker:
    ...
    def __init__(self, robot, color_key, arm=None):
        self.robot      = robot
        self.color_key  = color_key
        self.color_name = COLORS[color_key][0]
        self.box_color  = COLORS[color_key][1]
        self.arm        = arm              # ArmController (or None if no arm)
    ...
    def _run_avoiding(self, target):
        ...
        # The peek is asking: "is there still a wall to my side?"
        # Simple threshold: if sonar sees something within obstacle range,
        # the wall is still there. If it reads far, the wall has ended.
        # No reference-distance comparison — the frontal approach distance
        # and the sideways peek distance are fundamentally different
        # measurements that don't match within a tight tolerance.
        elif self.avoid_phase == 5:
            if elapsed >= SCAN_SETTLE_TIME:
                peek_dist = self._read_sonar_avg()

                if peek_dist <= OBSTACLE_DISTANCE_CM:
                    ...
                    # Wall ended — strafe to clear corner, then SEARCHING
    ...
    def _run_arrived(self, target):
        ...
        if time.time() - self.dance_step_start >= duration:
            ...
            if self.dance_step >= len(steps):
                # Dance done -- stop motors, kill LEDs
                self._stop()
                self._leds_off()

                # Place the block if arm is attached
                if self.arm is not None:
                    self.arm.place_sequence()

                self.dance_complete = True
                print('[ARRIVED] Complete. Press Q to exit.')
                return
            cmd, duration = steps[self.dance_step]
...
def main():
    ...
    # --- Build tracker with the chosen color + arm ---
    tracker = ColorBlockTracker(robot, color_key, arm=arm)
    ...
    robot.Ctrl_WQ2812_ALL(0, 0)
    robot.Ctrl_Ulatist_Switch(0)
    arm.close()
    camera.stop()
    cv2.destroyAllWindows()
    del robot
